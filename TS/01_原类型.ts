...
const obj: object = [1, 2, 3, 4]   // 像数组 函数  对象 Date Error class类 构造函数都是object类型
const Obj: Object = [1, 2, 3, 4]   // 大Object能存储能调用Object方法的类型 只有null和undefined不能存 它比小object还能存
...
let person: { name: string, age?: number, [key: string]: any }  // [key:string]:any 定义任意类型为string的属性并且值为任意类型 叫索引签名
person = { name: 'ming', hello: '123', key: 123 }  // ?表示可省，索引签名可以续写无数个
// 定义函数类型 ()里面定义参数 加上=>分割符 并确定返回值的类型
let cou: (a: number, b: number) => number  // 注意=> 是分隔符只是为了告诉你这个函数的返回值应该是什么类型的
cou = function (x, y) {
    return x + y
}
// 声明数组类型 
const arr1: string[] = ['c', 'l', 'h']  // 用类型加上[]表示字符串数组
const arr2: Array<number> = [1, 2, 3, 5]  // 两种写法 这种写法是泛型  之所以这里大写是因为没有小写的array
